...
public class ClienteDao {
	...
	public void inserir (Cliente cliente) {
		String sql ="insert into tb_clientes(nome,endereco,numero,email,genero,dataNascimento,prodInt)"
				+ "values(?,?,?,?,?,?,?)";
		//substituir valores 
		...
		try {
			...
			stmt.setString(7, cliente.getProdInt());
			//fechar sempre
	...
	// metodo Excluir 
		...
		public void atualizar (Cliente cliente) {
			String sql ="update tb_clientes set nome = ?, endereco = ?, numero = ?, email = ?, genero = ?, dataNascimento = ?, prodInt = ? where id = ? ";
			PreparedStatement stmt;
			try {
				stmt = conexao.prepareStatement(sql);
				stmt.setString(1, cliente.getNome());
